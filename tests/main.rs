...
fn main() -> anyhow::Result<()> {
    ...
    eprintln!();
    eprint!("[*] Checking number of marked call locations (default configuration)... ");
    ...
    eprintln!();
    eprint!("[*] Checking number of marked call locations (custom configuration)... ");
    assert_eq!(
...

    {
        ...
        eprint!("[*] Checking all custom configuration bookmarks are medium-priority... ");
        for i in 0..idb.bookmarks().len() {
            assert!(
                idb.bookmarks()
                    .get_description_by_index(i)
